...
struct oled_ssd1306
{
    ...
    i2c_master_dev_handle_t dev; // ONLY device handle
    uint8_t *fb;
    size_t fb_len;
};
...
{
    if (x < 0 || x >= o->cfg.width)
        return;
    if (page < 0 || page >= (o->cfg.height / 8))
        return;
                            ...
                            const oled_ssd1306_cfg_t *cfg)
{
    ...
    // Add the OLED to the EXISTING bus handle (Shared Bus 1)
    ESP_LOGI(TAG, "Adding OLED device to shared bus at address 0x%02X", o->cfg.i2c_addr);
    esp_err_t err = i2c_master_bus_add_device(bus, &dev_cfg, &o->dev);
    if (err != ESP_OK)
    {
...
esp_err_t oled_ssd1306_draw_text(oled_ssd1306_t *o, int x, int page_y, const char *text)
{
    if (!o || !text)
        return ESP_ERR_INVALID_ARG;
    if (page_y < 0 || page_y >= (o->cfg.height / 8))
        return ESP_ERR_INVALID_ARG;
